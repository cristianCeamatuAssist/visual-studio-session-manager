...
# Manages marker files for session status detection:
#   - No marker = working (orange)
#   - .waiting_ marker = needs input (green)
ACTION="$1"
...
if [ -n "$MARKER_ID" ]; then
  case "$ACTION" in
    ...
    resume|start)
      ...
      # Session terminated — clean up markers
      rm -f "$SESSIONS_DIR/$WAITING_PREFIX$MARKER_ID"
      ;;
  esac
fi
...
export class HookManager {
  async isInstalled(): Promise<boolean> {
    try {
      ...
      // All 5 hooks must be present for full detection
   ...
   * Returns a set of marker IDs (PIDs) that are confirmed waiting.
   */
  ...
  /**
   * Clean up stale marker files for processes that no longer exist.
   */
